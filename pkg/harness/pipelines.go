...
func ListInputSetsTool(config *config.Config, client *client.PipelineService) (tool mcp.Tool, handler server.ToolHandlerFunc) {
	return mcp.NewTool("list_input_sets",
			mcp.WithDescription("List input sets for pipelines in a Harness repository."),
			mcp.WithString("pipeline_identifier",
				mcp.Description("Optional pipeline identifier to filter input sets"),
			),
			mcp.WithString("search_term",
				mcp.Description("Optional search term to filter input sets"),
			),
			WithScope(config, true),
			WithPagination(),
		),
